  ...
  return (
    <div className="space-y-2">
      {items.map((item, index) => (
        <div key={index} className="glass-strong rounded-xl overflow-hidden">
          <button
            onClick={() => setOpenIndex(openIndex === index ? null : index)}
            className="w-full px-6 py-4 text-left font-display font-semibold flex justify-between items-center hover:bg-secondary/20 transition-colors"
          >
            {item.q}
            <span className={`transition-transform ${ openIndex === index ? 'rotate-180' : '' }`}>▼</span>
          </button>
          {openIndex === index && (
            <div className="px-6 py-4 border-t border-border/40 text-muted-foreground text-sm">
              {item.a}
            </div>
          )}
        </div>
      ...
      <section className="relative overflow-hidden bg-gradient-hero">
        ...
        <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 pt-16 pb-24 lg:pt-24 lg:pb-32 grid lg:grid-cols-2 gap-12 items-center">
          <div className="animate-fade-up">
            ...
            <h1 className="mt-5 font-display text-4xl sm:text-5xl lg:text-6xl font-bold leading-tight tracking-tight">
              Your <span className="gradient-text">AI fitness coach</span>,
              <br />
              available 24/7.
            </h1>
          ...
          <div className="relative animate-float hidden lg:block">
            ...
            <div className="relative rounded-3xl shadow-elegant border border-border/40 bg-gradient-to-br from-primary/10 to-secondary/10 h-96 flex items-center justify-center">
              <div className="text-center">
                <Dumbbell className="h-24 w-24 mx-auto text-primary opacity-50 mb-4" />
                <p className="text-muted-foreground">AI Fitness Coach</p>
              </div>
            </div>
          </div>
        </div>

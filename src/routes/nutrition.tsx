...
import { nutritionTips } from "@/lib/fitness-data";

export const Route = createFileRoute("/nutrition")({
  head: () => ({
    meta: [
      { title: "Nutrition — FitAI" },
      { name: "description", content: "Smart nutrition tips, hydration math, and a quick BMI calculator." },
    ],
  }),
...
function NutritionPage() {
  ...
  const bmiCategory = bmi == null ? null
    : bmi < 18.5 ? { label: "Underweight", color: "text-sky-300" }
    : bmi < 25 ? { label: "Healthy", color: "text-emerald-300" }
    : bmi < 30 ? { label: "Overweight", color: "text-amber-300" }
    ...
    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-12">
      ...
      <div className="grid lg:grid-cols-3 gap-6 mb-16">
        {nutritionTips.map((t, i) => (
          <article key={t.id} className="glass-strong rounded-2xl p-6 hover:shadow-glow hover:-translate-y-1 transition-all animate-fade-up" style={{ animationDelay: `${i * 60}ms` }}>
            <div className="flex items-start gap-4">
              <div className="text-4xl">{t.emoji}</div>
              <div>
                <p className="text-xs uppercase tracking-wider text-primary font-semibold">{t.category}</p>
      ...
      <section className="glass-strong rounded-3xl p-8 lg:p-12 max-w-3xl mx-auto">
        ...
        <Button onClick={calc} className="mt-5 bg-gradient-primary text-primary-foreground hover:opacity-90">
          Calculate BMI
        </Button>
